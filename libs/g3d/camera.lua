...
function camera.firstPersonMovement(dt)
    ...
    camera.moveX, camera.moveY = moveX, moveY
    ...
    if moveX ~= 0 or moveY ~= 0 then
        ...
        camera.position[1] = camera.position[1] + math.cos(fpsController.direction + angle) * camera.speed * dt
        camera.position[2] = camera.position[2] + math.sin(fpsController.direction + angle) * camera.speed * dt
        cameraMoved = true
    end
